...
* `8.0.5`, `latest` - Splunk universal forwarder base image [Dockerfile](https://github.com/splunk/docker-splunk/blob/master/enterprise/Dockerfile)
...
0. Enter the following command to pull the Splunk Enterprise version 8.0.5 image.<br>
   ...
   ```bash
   docker pull cribl/universalforwarder:latest
   ```
0. Run the Docker image.
   ...
   docker run --name splunkuniversalforwarder \
     ...
     --volume /var/log:/docker/log:ro \
       ...
       -d cribl/universalforwarder:6.5.3-monitor
   ```
...
The `8.0.5-monitor` tag ensures that the universal forwarder has the data inputs you need to get stats from a Docker container.

```bash
docker pull cribl/universalforwarder:8.0.5-monitor
```

### Pull the latest version of the image from this repository
The `8.0.5` and `latest` versions only have the forwarder and do not have any of the data inputs.
=======
...
```bash
docker pull cribl/universalforwarder:6.5.3-monitor
```
...
services:
  splunkuniversalforwarder:
    ...
    image: cribl/universalforwarder:8.0.5
...
`cribl/universalforwarder:8.0.5` and `cribl/universalforwarder:latest`
This is the default universal forwarder image.

`cribl/universalforwarder:6.5.3-monitor`
